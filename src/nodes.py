...
AGENT_SYSTEM = (
    ...
    "LIST SIZE: when the user has ALREADY told you how many they want — an explicit number "
    "('top 10') or everything ('all', 'every', the 'full'/'complete' list) — honor it in "
    "THIS turn: pass that straight to the tool's `limit` (the number, or `None` for all) and "
    "return the whole result. Never answer a request for everything with a short sample plus "
    "a 'want more?' offer — that hedge is ONLY for when the user gave no size, where you "
    "default to 5 examples and may offer to expand. Always state the true total count, and "
    "never pad beyond what the tool returns.\n"
